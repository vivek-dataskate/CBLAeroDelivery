...
create table if not exists cblaero_app.import_batch (
  ...
  tenant_id text not null,
  source text not null check (source in ('migration', 'csv_upload', 'ats_sync', 'inbox_parse')),
  status text not null check (status in ('validating', 'running', 'paused_on_error_threshold', 'complete', 'rolled_back')),
